...
**Hamstring Curl moved to Workout B, Suitcase Carry removed 2026-07-16**: John wanted to shorten the workout. Moved **Seated Hamstring Curl** (id 13) from a Lower Body staple (every session) to a Workout B-only rotator, in the slot previously held by **Suitcase Carry** (id 17, now retired/unused — card removed from HTML). Net effect: Lower Body drops from 4 exercises/session to 3 on both A and B days, total workout length drops from 20 to 19 exercises. Updated the Schedule tab's A/B explainer and "How A/B works" note to match. John also learned from his PT that he has an **anterior pelvic tilt**, and asked for exercise recommendations to address it before deciding what else to swap out — see Future Ideas below.

**Cat Stretch replaced with Posterior Pelvic Tilt 2026-07-16**: To address the anterior pelvic tilt his PT flagged, John chose to retire **Cat Stretch** (id 1, now retired/unused) from Warm-Up and replace it in the same slot with **Posterior Pelvic Tilt** (new id 32) — his reasoning was that several other exercises (Deep Core Activation, Dead Bug) already have him supine on the mat, so it fits naturally into the existing flow. Posterior Pelvic Tilt is the direct corrective move for APT and is pure flexion-based, consistent with his facet-arthritis/foraminal-narrowing restrictions. Total workout length stays at 19 exercises (one-for-one swap). The other APT candidate discussed — Standing Pelvic Tilt — is still unadded; see Future Ideas.
...
- **Anterior pelvic tilt correction (flagged by PT, 2026-07-16)**: John's PT identified an anterior pelvic tilt. **Posterior Pelvic Tilt was added 2026-07-16** (replaced Cat Stretch, id 32 — see above). Still uncommitted: **Standing Pelvic Tilt** (against a wall — same movement pattern, standing, no floor/kneeling/overhead involved) — a candidate if John wants a second APT-specific exercise, pending him picking what to swap out. Note: hip flexor stretching (Couch Stretch, Hip Flexor Stretch), hamstring work (Seated Hamstring Curl, Seated Hamstring Stretch), glute strengthening (Glute Bridge), and anti-extension core work (Dead Bug, Pallof Press/Warrior Press) already in the app all support APT correction too.
